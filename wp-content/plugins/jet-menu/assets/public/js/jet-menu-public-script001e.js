...
( function( $ ) {
	...
	window.jetMenu = {
			...
			Vue.component( 'mobilemenu', {
				...
				computed: {
					instanceClass: function() {
						let classes = [
							...
							`${ this.menuOptions.menuPosition }-position`,
						];
					...
					isBreadcrumbs: function() {
						return 'true' === this.menuOptions.useBreadcrumb && 0 !== this.breadcrumbsData.length ? true : false;
					},
				...
				methods: {
					...
					showTemplateContent: function() {
						this.$nextTick( () => {
							let $templateContainer = $( this.$refs['template-content'] ).find( '.jet-mobile-menu__template-content' );
		...
		elementorContentRender: function( $templateContainer, templateContent ) {
			...
			Promise.all( jetMenu.addedAssetsPromises ).then( value => {
